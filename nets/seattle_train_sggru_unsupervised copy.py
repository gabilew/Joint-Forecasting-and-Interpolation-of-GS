...
from data.Load_data import USA_data
...
def training_routine(args):
    ...
    #compute gft
    _, V = spectral_components(A,np.array(speed_matrix)[:1000] )
    freqs = np.arange(0,F,1)

    if args.e_opt:
        #_,V= np.linalg.eigh(A)
        #sample = greedy_e_opt(V[:,:S],S)
    ...
    pre_time = time.time()
    print(np.sum(speed_matrix.sum()), np.sum(sample), np.sum(V))
    ...
    os.mkdirs("models_and_losses/", exsist_ok=True)
