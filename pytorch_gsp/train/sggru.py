...
from torch_gsp.utils.gsp import Reconstruction
                                 



class SpectralGraphRNN(nn.Module):
    """
    SpectralGraphRNN [summary]
    ...
    Args:
        ...
        rnn (str, optional): predictive model. Defaults to 'gru'.
    """
    def __init__(self, V, sample,freqs, rnn = 'gru'):
        super(SpectralGraphRNN, self).__init__()
...
class SpectralGraphRNN2(nn.Module):
    def __init__(self, V, sample,freqs, rnn = 'gru'):

        super(SpectralGraphRNN2, self).__init__()
...
class model(nn.Module):
    ...
    def schedule(self,opt):
        ...
        if learning_rate > 1e-5:
            lamb = lambda epoch:  0.5 if epoch%20 == 0 else 1
